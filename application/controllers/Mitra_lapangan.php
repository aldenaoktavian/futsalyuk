...
class Mitra_lapangan extends CI_Controller {
	...
	{
		$this->load->view('mitra/mitra_register');	
	}
	...
	public function login()
	{
		...
		if($data_login != NULL){
			$nama_lapangan = db_get_one_data('nama', 'lapangan', array('id'=>$data_login['id_lapangan']));
			$newdata = array(
		        'id_mitra'  => $data_login['id'],
		        'email'     => $data_login['email'],
		        'nama_mitra' => $data_login['nama_lapangan'],
	...
	public function change_lap($id_lap="") {
		...
		if($id_lap != ""){
			...
			$nama_lapangan = db_get_one_data('nama', 'lapangan', array('id'=>$id_lapangan));
			$add_id_lap = array(
	...
	public function edit_profile() {
		...
		if($post){
			...
			$update = $this->M_mitra_lapangan->update_mitra_byid($data_update, $id_mitra);

			if($update != 0){
				$_SESSION['nama_mitra'] = $post['nama_lapangan'];
				echo '<script>alert("Berhasil Edit Profile."); window.location="'.base_url().'mitra_lapangan/edit_profile";</script>';
			} else{
				echo '<script>alert("Gagal Edit Profile. Silahkan coba kembali nanti."); window.location="'.base_url().'mitra_lapangan/edit_profile";</script>';
			}
		}

		$varcontent['data_mitra'] = $this->M_mitra_lapangan->get_data_mitra($id_mitra);

		$varcontent['view'] = 'edit_profile';	
		$this->load->view('mitra/template', $varcontent);
	}
	...
	public function add_lapangan() {
		...
		if($post){
			$last_id = db_get_one_data('MAX(id)', 'lapangan', null);
			$last_id = str_replace('LP1709', '', $last_id);
			$next_id = $last_id + 1;
			$next_id_length = strlen((string)$next_id);

			$new_id_lap = '';
			if($next_id_length == 1){
				$new_id_lap = 'LP170900'.$next_id;
			} else if($next_id_length == 2){
				$new_id_lap = 'LP17090'.$next_id;
			} else{
				$new_id_lap = 'LP1709'.$next_id;
			}
			...
			$insert = $this->M_mitra_lapangan->add_lapangan($data_lapangan);
			if($insert != 0){
				$_SESSION['id_lapangan'] = $new_id_lap;
				echo '<script>alert("Berhasil Tambah Lapangan."); window.location="'.base_url().'mitra_lapangan";</script>';
			} else{
				echo '<script>alert("Gagal Tambah Lapangan. Silahkan coba kembali nanti."); window.location="'.base_url().'mitra_lapangan/change_lap";</script>';
			}
		}
	}
	...
	public function search_transaksi_booking() {
		...
		$type = $this->M_mitra_lapangan->get_type($id_lapangan, $this->input->post('tipe_lapangan'));
		...
		for($i=9; $i<=22; $i++){
			...
			if($transaksi != NULL){
				...
				if($transaksi['transaksi_status'] == 1){
					$status_booking = 'Digunakan ( '.$transaksi['kode_booking'].' )';
				...
				if($this->input->post('startdate') < date('Y-m-d') && $start < date('H')){
					$html .= '<div class="transaksi_item_left">
					            <label class="label_time">'.$start.':00 - '.$end.':00</label><br/>
					            <span></span>
					          </div>
			...
			if($data_transaksi['id_user'] != 0){
				if($post['transaksi_status'] == 1){
					...
					if($update_transaksi != 0){
						$update_saldo_member = $this->M_mitra_lapangan->update_member($data_member_saldo, 'user_id', $data_transaksi['user_id']);
