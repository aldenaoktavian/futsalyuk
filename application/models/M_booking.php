...
class M_booking extends CI_Model {
    ...
    public function search_tipe_lapangan($id_lapangan,$jam,$tanggal)
    {
        ...
        $queryRec = $this->db->query($sql,array($id_lapangan,$tanggal,$jam));
        return $queryRec;
    }
    ...
    public function cancel_old_transaction()
    {
        $old_transaction = $this->db->query("SELECT transaksi_id FROM transaksi WHERE tanggal < '".date('Y-m-d')."' AND transaksi_status = 0")->result_array();

        if($old_transaction != NULL){
            foreach($old_transaction AS $transaksi){
                $update = array(
                        'transaksi_status'  => 2
                    );
                $this->db->update('transaksi', $update, array('transaksi_id'=>$transaksi['transaksi_id']));
            }
        }
    }
}
